 ...
 * Miner
 **/
function Miner(rewardType, id, login, pass, ip, port, agent, workerName, startingDiff, noRetarget, pushMessage){
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            miner = new Miner(rewardType, minerId, login, pass, ip, port, params.agent, workerName, difficulty, noRetarget, pushMessage);
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    if (slushMiningEnabled) {
        ...
        updateScore = ['hincrbyfloat', config.coin + ':scores:' + rewardType + ':roundCurrent', miner.login, job.score]
    }

    let redisCommands = [
        updateScore,
        ['hincrby', config.coin + ':shares_actual:' + rewardType + ':roundCurrent', miner.login, job.difficulty],
        ['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty, miner.login, dateNow, rewardType].join(':')],
    ...
    if (miner.workerName) {
        redisCommands.push(['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty, miner.login + '~' + miner.workerName, dateNow, rewardType].join(':')]);
    ...
    if (blockCandidate){
        redisCommands.push(['hset', config.coin + ':stats', 'lastBlockFound' + rewardType, Date.now()]);
        redisCommands.push(['rename', config.coin + ':scores:prop:roundCurrent', config.coin + ':scores:prop:round' + job.height]);
        redisCommands.push(['rename', config.coin + ':scores:solo:roundCurrent', config.coin + ':scores:solo:round' + job.height]);
        redisCommands.push(['rename', config.coin + ':shares_actual:prop:roundCurrent', config.coin + ':shares_actual:prop:round' + job.height]);
        redisCommands.push(['rename', config.coin + ':shares_actual:solo:roundCurrent', config.coin + ':shares_actual:solo:round' + job.height]);
        if (rewardType === 'prop') {
            redisCommands.push(['hgetall', config.coin + ':scores:prop:round' + job.height]);
            redisCommands.push(['hgetall', config.coin + ':shares_actual:prop:round' + job.height]);
        }
        if (rewardType === 'solo') {
            redisCommands.push(['hget', config.coin + ':scores:solo:round' + job.height, miner.login]);
            redisCommands.push(['hget', config.coin + ':shares_actual:solo:round' + job.height, miner.login]);
        }
    }
